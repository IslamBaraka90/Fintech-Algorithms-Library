...
- **717 topics** across 19 domains and 90 families
- **623 verified** — arithmetic replayed and asserted on every build; 94 at
  `contract`, where the signature is checked but nothing asserts the numbers
- **717 / 717** carry a validated `api:` contract
- **717 / 717** carry an executed worked example
- **1,352 tests pass**, `tsc` builds clean, zero runtime dependencies
- Published: `0.13.1`. `main` is ahead of it — see [CHANGELOG.md](CHANGELOG.md).
...
```bash
npm pack
npm install /path/to/fintech-algorithms-0.13.2.tgz
```
