...
export function logLoss(inputs: AnyRecord): AnyRecord {
  const rows = records(inputs, { probability: true });
  // The default applies only when the key is absent. A present-but-null epsilon
  // is an invalid value, not a request for the default, and is refused here as
  // the Python reference refuses it.
